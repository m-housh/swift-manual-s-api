...
struct OneWayIndoorRoute: Renderable {
  ...
  var description: Node {
    row {
      [
        .p("This route is used for one way interpolation of manufacturer's capacities at indoor design conditions."),
        .br,
        .br,
        .h6(attributes: [],
          "Example: My summer outdoor design is 95° dry-bulb and my indoor design is 63° wet-bulb"
        ),
        .ul(attributes: [.class("ms-5")],
          .li(.text("The manufacturer's published outdoor data is equal to the design conditions.")),
          .li(.text("The manufacturer's published indoor data is 67° and 62° wet-bulb, so interpolation is needed to calculate the capacity at 63° wet-bulb."))
        )
      ]
    }
  }
...
fileprivate func renderCooling(_ route: ServerRoute.Documentation.Route.Interpolation.Cooling) async throws -> Node {
  switch route {
  case .oneWayIndoor:
...
func renderInterpolateRoute(_ route: ServerRoute.Documentation.Route.Interpolation) async throws -> Node {
  switch route {
  case .home:
